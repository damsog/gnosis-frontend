    ...
    return (
        <div className='flex flex-col md:grid md:grid-cols-4 gap-4'>
            <div className='md:col-span-1'>
                <h2 className='text-gray-200 text-4xl'>{capitalize(group.name)}</h2>
                <p className=' my-4 text-gray-400 text-sm'>{group.description}</p>
                <div className=''>
                    <div className='flex justify-between items-center'>
                        <h5 className='text-gray-200 text-2xl'>Dataset</h5>
                        <h5 className='my-2 text-gray-300 p-1'>{group.dataset ? <span className='bg-green-700 rounded-lg p-1'>Generated<GiCheckMark className='inline-flex'/></span> :
                    ...
                    <div className='flex justify-around'>
                        ...
                        <h5 className="rounded-lg p-2 bg-[#2b2532] hover:bg-[#3f3847]
                                    active:translate-y-1 text-xl cursor-pointer text-gray-300 inline-flex" onClick={() => setShowing(!showing)}>Show <IoTrashOutline/></h5>
                    </div>
                    <div>
                        <label className="rounded-lg p-1 text-gray-400 bg-[#2b2532] w-10 h-4">{(group.dataset && showing) ? group.dataset : ""}</label>
                    </div>
                </div>
            </div>
